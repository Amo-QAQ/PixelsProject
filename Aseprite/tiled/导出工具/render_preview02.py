...
for ts_elem in root.findall("tileset"):
    ...
    spritesheet = None
    tile_images = {}  # collection: local_id -> (image, px, py, w, h)
...
# Actual gid ranges from adjacent firstgids
...
print(f"\nRendering {len(layers)} layers...")

# Animation cycles (only character 8-frame anims here)
anim_lengths = set()
...
print(f"Animation cycles: {sorted(anim_lengths)} frames each")
print(f"LCM = {total_frames} frames, {frame_duration_ms}ms/frame, loop {total_frames * frame_duration_ms / 1000:.1f}s")
